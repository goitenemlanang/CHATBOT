...
def get_response(chat, slots, count):
    ...
    if len(slots) == 1:  # thieu state hoặc intent
        ...
        elif slots[0] == "greeting" and slots[1] in state_tags:
            str = f"Bạn cần hỏi thông tin nào của tour {slots[1]}?"
            slots.pop(0)
            return str
        elif slots[0] not in list_key_db and slots[1] in state_tags:
            for intent in intents["intents"]:
                if slots[0] == intent["tag"]:
                    slots.pop(0)
                    return f"{random.choice(intent['responses'])}"
